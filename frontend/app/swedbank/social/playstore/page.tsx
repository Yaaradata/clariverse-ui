...
import {
  getPlayStoreKPIs,
  getPlayStoreViralityTopics,
  getPlayStoreReviewAlerts,
  getPlayStoreModerationDataset,
  getPlayStoreModerationAreaData,
  getPlayStoreSentimentLevelTimeline,
...
} from '@/lib/swedbank/social/playstore';
...
export default function PlayStoreDashboard() {
  const [kpis, setKpis] = useState<PlayStoreKPI[]>([]);
  const [viralityTopics, setViralityTopics] = useState<PlayStoreViralityTopic[]>([]);
  const [reviewAlerts, setReviewAlerts] = useState<PlayStoreReviewAlert[]>([]);
  const [selectedCategoryKey, setSelectedCategoryKey] = useState<'moderation' | 'feature' | 'appreciation' | null>(null);
  const [selectedPoint, setSelectedPoint] = useState<{ date: string; levelKey: (typeof PLAYSTORE_SENTIMENT_LEVELS)[number]['key'] } | null>(null);

  const moderationDataset = useMemo(() => getPlayStoreModerationDataset(), []);
  const moderationAreaData = useMemo(() => getPlayStoreModerationAreaData(), []);
  const sentimentLevelTimeline = useMemo(() => getPlayStoreSentimentLevelTimeline(), []);
  const topicVolumeSplit = useMemo<PlayStoreTopicVolumeSplitEntry[]>(
    () => getPlayStoreTopicVolumeSplit(),
    []
  );
  ...
  const selectedDetail = useMemo(() => {
    ...
    const levelIndex = PLAYSTORE_SENTIMENT_LEVELS.findIndex(level => level.key === selectedPoint.levelKey);
    if (levelIndex === -1) return null;
    const level = PLAYSTORE_SENTIMENT_LEVELS[levelIndex];
  ...
  useEffect(() => {
    setKpis(getPlayStoreKPIs());
    setViralityTopics(getPlayStoreViralityTopics());
    setReviewAlerts(getPlayStoreReviewAlerts());
  }, []);
      ...
      <div className="space-y-6">
        ...
        <Card className={SOCIAL_CARD_BASE}>
          <CardHeader>
            ...
            <CardDescription className="text-gray-400">
              Surface Play Store moderation hotspots and feature demand emerging from review streams
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-6">
                ...
                {moderationDataset.summaries.map(summary => (
                  ...
                  >
                    <div className="flex items-center justify-between text-xs uppercase tracking-wide text-gray-500">
                      ...
                      <span>{PLAYSTORE_SENTIMENT_LEVELS.find(level => level.key === summary.dominantSentiment)?.label}</span>
                    </div>
                  ...
                  {moderationAreaData.length > 0 ? (
                    <div className={SOCIAL_CHART_SURFACE}>
                      <ResponsiveContainer width="100%" height={320}>
                        ...
                        <defs>
                          <linearGradient id="playStoreModerationGradient" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="5%" stopColor="#f97316" stopOpacity={0.6} />
                            <stop offset="95%" stopColor="#f97316" stopOpacity={0.05} />
                          </linearGradient>
                          <linearGradient id="playStoreFeatureGradient" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="5%" stopColor="#38bdf8" stopOpacity={0.6} />
                            <stop offset="95%" stopColor="#38bdf8" stopOpacity={0.05} />
                          </linearGradient>
                          <linearGradient id="playStoreAppreciationGradient" x1="0" y1="0" x2="0" y2="1">
                        ...
                        <Area
                          ...
                          fill="url(#playStoreModerationGradient)"
                          activeDot={{ r: 6 }}
                        ...
                        <Area
                          ...
                          fill="url(#playStoreFeatureGradient)"
                          activeDot={{ r: 6 }}
                        ...
                        <Area
                          ...
                          fill="url(#playStoreAppreciationGradient)"
                          activeDot={{ r: 6 }}
                    ...
                    <div className="text-xs text-gray-400 italic">
                      Not enough review signals yet to visualise moderation, feature demand, and appreciation themes.
                    </div>
                  )}
                </div>
                                      ...
                                      <div className="space-y-3">
                                        <div>
                                          ...
                                          <p className="text-[11px] text-gray-400">
                                            Weighted by frequency across Play Store feedback
                                          </p>
                                        </div>
        ...
        <Card className={SOCIAL_CARD_BASE}>
          <CardHeader>
            ...
            <CardDescription className="text-gray-400">
              Play Store review clusters ranked by volume of uplifting versus detracting narratives
            </CardDescription>
          </CardHeader>
        ...
        <Card className={SOCIAL_CARD_BASE}>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-white text-lg">
              Daily Play Store Reviews by Sentiment Level
            </CardTitle>
            <CardDescription className="text-gray-400">
              Sentiment intensity across Play Store reviews with helpful-vote momentum
            </CardDescription>
          </CardHeader>
          <CardContent className="pt-2 pb-6">
                  ...
                  >
                    ...
                    <Legend
                      ...
                      formatter={(value: string) => {
                        const match = PLAYSTORE_SENTIMENT_LEVELS.find(level => level.key === value);
