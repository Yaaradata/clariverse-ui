...
export function FCIKPICards({ data, isDarkMode = false }: FCIKPICardsProps) {
  const kpiData = {
    totalInteraction: {
      ...
      trend: 3.2,
      totalVolume: 124567,
      lastWeekComparison: '+3.2K',
      sparkline: [45, 52, 48, 61, 55, 68, 75, 87]
    },
    fciRate: {
      value: 18.5,
      trend: -2.3,
      channels: { voice: 32, email: 28, tickets: 18, social: 12, chat: 10 },
      actualResolutions: 2314,
      industryAvg: 16.2,
      sevenDayAvg: 17.9
    },
    crossChannelReport: {
    ...
    escalationRate: {
      value: 12.8,
      trend: -1.2,
      casesEscalatedToday: 1587,
      avgEscalationTime: 4.2,
      mostEscalated: { category: 'Billing issues', percentage: 34 }
    },
    riskSignal: {
      fraud: { percentage: 3.2, cases: 402 },
      outage: { percentage: 0.8, cases: 95 },
      compliance: { percentage: 5.1, cases: 602 },
      totalFlagged: 12543,
      highPriority: 156
    },
    customerSentiment: {
      ...
      <div className="col-span-3 grid grid-cols-3 gap-4">
        ...
        <div className="border rounded-xl p-4" style={cardStyle}>
          ...
          <div className="space-y-1.5 mt-2">
            ...
            <p className="text-sm" style={{ color: '#939394' }}>Industry avg: {kpiData.fciRate.industryAvg}%</p>
            <p className="text-sm" style={{ color: '#939394' }}>7-day average: {kpiData.fciRate.sevenDayAvg}%</p>
            <div className="grid grid-cols-2 gap-x-1 text-sm pt-1" style={{ color: '#939394' }}>
              <span className="flex items-center gap-0.5"><Phone className="w-3 h-3" />{kpiData.fciRate.channels.voice}%</span>
              <span className="flex items-center gap-0.5"><Mail className="w-3 h-3" />{kpiData.fciRate.channels.email}%</span>
            </div>
          </div>
        </div>
        ...
        <div className="border rounded-xl p-4" style={cardStyle}>
          ...
          <div className="space-y-1.5 mt-2">
            <div className="flex items-center justify-between text-sm">
              <span className="flex items-center gap-0.5" style={{ color: '#939394' }}><AlertOctagon className="w-3 h-3" style={{ color: '#ef4444' }} />Fraud</span>
            ...
            <div className="flex items-center justify-between text-sm">
              <span className="flex items-center gap-0.5" style={{ color: '#939394' }}><Zap className="w-3 h-3" style={{ color: '#f59e0b' }} />Outage</span>
            ...
            <div className="flex items-center justify-between text-sm">
              <span className="flex items-center gap-0.5" style={{ color: '#939394' }}><Shield className="w-3 h-3" style={{ color: '#5332FF' }} />Compliance</span>
            ...
            <div className="pt-1.5 border-t" style={{ borderColor: isDarkMode ? '#1f1f1f' : '#E5E5E5' }}>
              <p className="text-sm font-semibold mt-1.5" style={{ color: isDarkMode ? '#FFFFFF' : '#010101' }}>Total flagged: {formatNumber(kpiData.riskSignal.totalFlagged)}</p>
        ...
        <div className="border rounded-xl p-4" style={cardStyle}>
          ...
          <div className="flex gap-0.5 h-2 rounded overflow-hidden mb-2.5">
