  ...
  return (
    <div className="flex flex-col gap-6 h-full min-h-0 min-w-0 w-full">
      <Card className={SOCIAL_CARD_BASE}>
        ...
        <CardContent className="xl:max-h-[460px] xl:overflow-y-auto pr-2">
                        ...
                        <div className="flex-1 min-w-0 space-y-2">
                          <div className="flex items-center gap-2 flex-wrap">
                            {(alert.urgency === 'critical' || alert.urgency === 'high') && (
                              <span className="text-[10px] font-semibold text-orange-300 bg-orange-500/20 px-2 py-0.5 rounded flex items-center gap-1 uppercase tracking-wide flex-shrink-0">
                            ...
                            <span className="text-[11px] text-gray-400 flex items-center gap-1">
                              ...
                              <span>{alert.firstDetected}</span>
                            </span>
                          </div>
                          <div className="flex items-center gap-2 flex-wrap">
                            <h4 className="text-sm font-semibold text-white leading-tight break-words">{alert.topic}</h4>
                            {alert.verified && <Star className="h-3.5 w-3.5 text-sky-300 fill-sky-300 flex-shrink-0" />}
                          </div>
                          <div className="text-xs text-gray-400 flex items-center gap-2 flex-wrap">
                            <span className="text-gray-300 font-medium">{alert.author}</span>
                            <span className="text-gray-500">{alert.handle}</span>
                          </div>
                          <p className="text-xs text-gray-300 line-clamp-2 break-words leading-relaxed">{alert.summary}</p>
                        </div>
                        <span className={`text-[10px] px-2.5 py-1 rounded-full uppercase tracking-wide flex-shrink-0 ${urgencyChip(alert.urgency)}`}>
                          {alert.urgency}
                        </span>
                      </div>
                      <div className="flex flex-wrap items-center gap-x-3 gap-y-2 text-xs text-gray-400 mt-3 pt-2 border-t border-white/10">
                        <span className="flex items-center gap-1">
                          <Hash className="h-3 w-3 text-purple-300 flex-shrink-0" />
                          <span className="font-medium">{alert.impactedHandles}</span> handles
                        </span>
                        <span className="flex items-center gap-1">
                          <Eye className="h-3 w-3 text-blue-300 flex-shrink-0" />
                          <span className="font-medium">{alert.impressions.toLocaleString()}</span> impressions
                        </span>
                        <span className="flex items-center gap-1">
                          <Heart className="h-3 w-3 text-rose-300 flex-shrink-0" />
                          <span className="font-medium">{alert.likes.toLocaleString()}</span> likes
                        </span>
                        <span className="flex items-center gap-1">
                          <Repeat2 className="h-3 w-3 text-sky-300 flex-shrink-0" />
                          <span className="font-medium">{alert.reposts.toLocaleString()}</span> reposts
                        </span>
                      </div>
                    </div>
                  </TooltipTrigger>
      ...
      <Card className={`${SOCIAL_CARD_BASE} flex flex-col flex-1 min-h-[420px]`}>
        ...
        <CardContent className="px-6 pb-6 space-y-3 overflow-y-auto flex-1 min-h-0">
          {creatorWatchlist.map((creator) => (
            <div key={creator.id} className={`${SOCIAL_PANEL_BASE} space-y-2.5 p-4`}>
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <h4 className="text-sm font-semibold text-white break-words">{creator.name}</h4>
                  <p className="text-xs text-gray-400 mt-0.5 break-words">{creator.handle}</p>
                </div>
                <div className="text-right text-xs text-gray-400 whitespace-nowrap leading-relaxed flex-shrink-0">
                  <div className="font-medium">{creator.followers.toLocaleString()} followers</div>
                  <div className="font-medium">{creator.avgEngagement}% engagement</div>
                </div>
              </div>
              <p className="text-xs text-gray-400 leading-relaxed break-words">{creator.lastPost}</p>
            </div>
          ))}
        </CardContent>
      </Card>
