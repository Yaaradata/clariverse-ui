...
export function ExecutiveTile({
  ...
  onClick?: () => void;
  /** AP-001 / AP-002 — primary triad tile reads largest. */
  isPrimary?: boolean;
}): React.ReactElement {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "baseline", gap: 10, flexWrap: "wrap" }}>
        <span className="lisn-num" style={{ fontSize: isPrimary ? 38 : 34, fontWeight: 800, color: cssVar("text-primary"), lineHeight: 1 }}>
          {tile.primaryValue}
        </span>
        <span className="lisn-num" style={{ fontSize: 12, fontWeight: 600, color: deltaColor(tile.deltaTone) }}>
          {tile.delta}
        </span>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1fr 80px", gap: 8, alignItems: "end" }}>
        <MiniSparkline data={tile.spark} color={accent} height={48} />
      ...
      >
        ...
        <p style={{ margin: 0, fontSize: 12.5, color: cssVar("text-secondary"), lineHeight: 1.5 }}>{tile.aiInsight}</p>
