...
export function Sidebar({
  ...
  const { onExit, industryName, roleName, industryColor } = useDashboardShell();
  ...
  return (
    ...
    >
      ...
      >
        {expanded ? (
          <>
            ...
            <div style={{ fontSize: 13, color: cssVar("text-secondary"), marginTop: 2 }}>
              Fluid CX
            </div>
          </>
        ) : (
      ...
      <div
        style={{
          padding: expanded ? "12px 14px" : "10px 8px",
          borderBottom: `1px solid ${cssVar("border")}`,
          display: "flex",
          flexDirection: "column",
          gap: expanded ? 8 : 6,
      ...
      >
        <ContextRow expanded={expanded} color={industryColor} label={industryName} />
        <ContextRow expanded={expanded} color={cssVar("accent")} label={roleName} accent />
      </div>
          ...
          return (
            <li key={screen.id}>
              ...
              >
                ...
                />
                {expanded && <span style={{ whiteSpace: "nowrap" }}>{screen.shortLabel}</span>}
              </button>
            </li>
          );
        })}
