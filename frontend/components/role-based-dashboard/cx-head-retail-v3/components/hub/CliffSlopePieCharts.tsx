...
import { ANXIETY_CLIFF_EVENTS, ANXIETY_SLOPE_EVENTS } from "../../lib/cxHeadRetailV3AnxietyData";
...
function SliceLegend({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      {items.map((item) => {
        const selected = selectedKey === item.k;
          ...
          >
            ...
            <div style={{ textAlign: "right", flexShrink: 0 }}>
              <div className="lisn-num" style={{ fontSize: 12, fontWeight: 700, color: colors.textPrimary, lineHeight: 1.2 }}>
                {mode === "cliff" ? item.v : anxietyFmt(item.v)}
              </div>
...
export function CliffSlopePieCharts({ mode }: { mode: CliffSlopeEventMode }): React.ReactElement {
  ...
  const total = useMemo(
    () => (mode === "cliff" ? ANXIETY_CLIFF_EVENTS : SLOPE_ITEMS).reduce((sum, e) => sum + e.v, 0),
    [mode],
  );
  ...
  }, [colorMap, colors.accent, source, total]);
