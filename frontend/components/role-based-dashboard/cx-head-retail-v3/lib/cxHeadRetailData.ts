...
export const EXEC_SUMMARY = {
  critical: "Checkout payment · WH-East refund backlog · Platinum churn window",
  focus: "D07 fulfilment cluster · seller trust erosion · promo-code failures at cart",
  stable: "5 of 8 fulfilment nodes flat · chat FCR holding at 78%",
  aiLine:
    "Checkout failure corroborated on app, chat, and social; fulfilment peers flat outside WH-East.",
...
export const EXECUTIVE_TILES: ExecutiveTileData[] = [
  ...
  {
    id: "statutory-clock",
    breakingIssue: "Consumer-rights deadline queue",
    owner: "Legal · Grievance",
    severity: "critical",
    primaryValue: "4",
    primaryLabel: "within 6h of deadline",
    delta: "Refund SLA breach leading at T−8h",
    deltaTone: "warn",
    onset: "T−8h closest",
    channels: ["Grievance desk", "Email"],
    spark: [1, 2, 2, 3, 3, 4, 4],
    recommendedAction:
      "Prioritise by deadline — human approve before escalate.",
    aiInsight:
      "Marketplace refund-policy complaints approaching statutory response windows.",
    confidence: "High",
  },
];

export const KPI_RIBBON = [
  { label: "Contact per order", value: "13.8 / 1k", delta: "+0.6 vs last week", tone: "warn" as const },
  { label: "Active spike themes", value: "5 live", delta: "+2 vs yesterday", tone: "warn" as const },
  { label: "NPS delta", value: "−5 pts", delta: "Premium-led", tone: "down" as const },
  { label: "Refund SLA", value: "58%", delta: "−11 pts WoW", tone: "down" as const },
];
...
/** Top clock-proximity rows for command-centre tile drill — within 6h first, then next by hours left. */
export function getStatutoryClockDrillQueue(): StatutoryQueueItem[] {
  const byProximity = [...STATUTORY_CLOCK_RUNWAYS].sort((a, b) => a.hoursLeft - b.hoursLeft);
  const withinSix = byProximity.filter((r) => r.hoursLeft <= 6).map((r) => r.id);
  const rest = byProximity.map((r) => r.id).filter((id) => !withinSix.includes(id));
  const ids = [...withinSix, ...rest].slice(0, 4);
  return ids
    .map((id) => getStatutoryItemById(id))
    .filter((item): item is StatutoryQueueItem => item !== undefined);
}
