...
/**
 * Neo Group · Head of Client Experience — Neogroup-only fork of the CX Command screen.
 * Route: /role-based/neogroup/head_client_experience
 * Do NOT edit nuvamapage.tsx for Neo localization — keep this file as the sole UI copy.
 */
...
import {
  NEOGROUP_D1_RETENTION_AI,
  NEOGROUP_D1_RETENTION_DETAILS,
  NEOGROUP_D2_PROMISE_AI,
  NEOGROUP_D2_PROMISE_DETAILS,
} from "@/lib/role-based-dashboard/neogroupClientExperienceAISummaryData";
import {
  Bar,
  BarChart,
  CartesianGrid,
...
/* shared chrome */
function NeoTopBar({ firmName }: { firmName: string }) {
  return (
    <div style={{ marginBottom: 14 }}>
      ...
      <div style={{ color: T.muted, fontSize: 12.5, marginTop: 2 }}>
        {firmName} · Client Experience Office · reporting to the Head of Wealth
      </div>
    </div>
  );
}
    ...
    <>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, flexWrap: "wrap", marginBottom: 14 }}>
        <div style={{ minWidth: 260 }}>
          ...
          {sub ? <div style={{ color: T.sub, fontSize: 15, marginTop: 4, maxWidth: 880, lineHeight: 1.5 }}>{sub}</div> : null}
        </div>
...
/* mini viz — fixed SVG / chart sizes (no ResponsiveContainer measure loops) */
...
function MiniSpark({ data, c, h = 52 }: { data: TrendPoint[]; c: string; h?: number }) {
  ...
  const points = vals
    ...
    .join(" ");
  return (
    <div style={{ height: h, minWidth: 0, width: "100%" }}>
      <svg width="100%" height={h} viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" aria-hidden>
        <polyline fill="none" stroke={c} strokeWidth="2.4" points={points} vectorEffect="non-scaling-stroke" />
      </svg>
    </div>
  );
}
...
function MiniGauge({ label, topLabel, value, color, suffix = "%" }: { label: string; topLabel?: string; value: number; color: string; suffix?: string }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: 1, minWidth: 0 }}>
      ...
      <div style={{ position: "relative", width: "100%", height: 48, display: "flex", justifyContent: "center" }}>
        <RadialBarChart
          width={120}
          height={48}
        ...
        >
          ...
          <RadialBar dataKey="value" cornerRadius={4} background={{ fill: `${T.inner}90` }} isAnimationActive={false} />
        </RadialBarChart>
...
const EXEC_PULSE = [
  ...
  { label: "3. What's working", text: "Automation is clearing routine admin: 62% of eligible requests now resolve without an RM, and self-service is climbing - returning relationship time to advice. The cost proof sits in the service-economics drill, the one place fulfilment data is joined." },
];

function NeoExecutivePulse() {
  return (
...
const MONITOR_ALERTS: MonitorAlert[] = [
  { id: "exit-intent", title: "Exit-Intent Language Spike", sev: "critical", sevLabel: "Retention", variant: "critical", feed: "Voice + social media",
    fields: [["Cohort", "South - Core-HNI"], ["Window", "Last 7 days"]],
    stats: [["Exit-language", "47 clients"], ["Baseline", "6 clients"], ["Sentiment", "-0.58 shift"]],
    ai: "Cluster of 'move my portfolio', 'close relationship', 'yield below expectation' concentrated in one region-segment cell. Draft evidence pack for the RM team - never auto-contact.", route: "rm" },
  ...
  { id: "promise-break", title: "Service-Promise Slippage", sev: "critical", sevLabel: "Promise", variant: "critical", feed: "trouble tickets + Voice",
    fields: [["Cohort", "RM call-backs"], ["Window", "This week"]],
  ...
  { id: "deflection", title: "Self-Service Deflection Rising", sev: "later", sevLabel: "Payoff", variant: "voice", feed: "chatmessages",
    fields: [["Cohort", "Balance + statement"], ["Window", "This week"]],
    stats: [["Deflection", "+14% WoW"], ["Cost-to-serve", "Falling"], ["Channel", "chatmessages"]],
    ai: "Routine balance and statement queries are resolving in self-service - keeping them off the RM's desk and returning relationship time to advice while lowering cost-to-serve. Extend the pattern to report requests next.", route: "digital" },
];
  ...
  return (
    <div style={{ marginTop: 22 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
        <div style={{ color: T.gold, fontSize: 12, letterSpacing: ".08em", textTransform: "uppercase", fontWeight: 900 }}>Today's Client-Signal Monitor</div>
        <span style={{ fontSize: 9, fontWeight: 900, letterSpacing: ".08em", textTransform: "uppercase", border: "1px solid #7e1f1f", background: "#301818", color: "#ff4444", borderRadius: 999, padding: "4px 10px" }}>Experience Alerts</span>
      </div>
      <div style={{ display: "flex", gap: 12, overflowX: "auto", paddingBottom: 10, alignItems: "stretch" }}>
          ...
          return (
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                <div style={{ fontSize: 15, lineHeight: 1.2, fontWeight: 900, color: T.text, flex: 1, minWidth: 0 }}>{a.title}</div>
                <span style={{ fontSize: 9, textTransform: "uppercase", fontWeight: 900, borderRadius: 999, padding: "4px 8px", whiteSpace: "nowrap", color: sv.color, background: sv.bg, border: `1px solid ${sv.border}`, flexShrink: 0 }}>{a.sevLabel}</span>
              </div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 5, alignItems: "center" }}>
                <span style={{ fontSize: 8.5, fontWeight: 800, letterSpacing: ".04em", textTransform: "uppercase", padding: "2px 7px", borderRadius: 999, color: T.muted, background: T.inset, border: `1px solid ${T.inner}` }}>{a.feed}</span>
                {a.needsExtraFeed ? <span style={{ fontSize: 8.5, fontWeight: 800, letterSpacing: ".04em", textTransform: "uppercase", padding: "2px 7px", borderRadius: 999, color: T.amber, background: `${T.amber}14`, border: `1px solid ${T.amber}40` }}>hand-off flag</span> : null}
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                {a.fields.map(([k, v]) => (
                  <div key={k} style={{ display: "grid", gridTemplateColumns: "88px 1fr", gap: 6, fontSize: 11, alignItems: "baseline" }}>
                    <span style={{ color: "#8c8c95", textTransform: "uppercase", letterSpacing: ".05em", fontWeight: 800 }}>{k}</span>
                    <span style={{ textAlign: "right", fontWeight: 700, color: "#fff", lineHeight: 1.25 }}>{v}</span>
                  </div>
                ))}
              </div>
              <div style={{ background: "#191919", border: "1px solid #333", borderRadius: 8, padding: "8px 10px", boxSizing: "border-box" }}>
                {monitorStatRows(a.stats).map(([k, v], i, rows) => (
                  ...
                  >
                    ...
                    <b style={{ textAlign: "right", color: "#fff", fontFamily: MONO, fontWeight: 700, whiteSpace: "nowrap" }}>{v}</b>
                  </div>
                ))}
              </div>
              <div
              ...
              >
                <span style={{ display: "block" }}>{a.ai}</span>
...
function NeoOverview({ go, firmName }: { go: NavigateFn; firmName: string }) {
  return (
    <div className="fade">
      <NeoTopBar firmName={firmName} />
      <NeoExecutivePulse />
      <div className="overview-cards">
        ...
        <ExecutiveQuestionCard accent="green" iTone="green" icon={<TrendingUp size={18} />} title="Is service paying off?" subtitle="Automation - deflection - cost-to-serve"
          ...
          aiText="Automation is clearing routine admin off the RM's desk: 62% of eligible requests now resolve without an agent, and self-service is climbing - returning relationship time to advice. The cost proof sits in the service-economics drill, the one place fulfilment data is joined."
...
const D1_SEGMENT_SENT: [string, string, number, number, number][] = [
  ...
  ["Emerging HNI", "24,860 int", 44, 28, 28],
  ["Digital-Retail", "9,910 int", 41, 29, 30],
];
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: 12 }}>
      <CCWell accent={T.red} title="Exit-intent monitor" sub="attrition-risk language - cohort level, never named client">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 8 }}>
          {[["South - Core-HNI", "47", T.red], ["Alternatives Advisory", "14", T.amber], ["West - HNI", "9", T.yellow], ["Other cells", "6", T.green]].map(([l, v, c]) => (
...
const D1_AI = NEOGROUP_D1_RETENTION_AI;
const D1_AI_DETAILS = NEOGROUP_D1_RETENTION_DETAILS;
...
const RADAR_ROWS: { label: string; cells: number[] }[] = [
  ...
  { label: "Emerging HNI", cells: [58, 46, 40, 34] },
  { label: "Digital-Retail", cells: [52, 48, 44, 41] },
];
...
const AT_RISK: { cohort: string; driver: string; signal: string; trend: string; sev: string; route: string; rmTeam: string }[] = [
  ...
  { cohort: "Alternatives Advisory", driver: "Yield / performance-concern language", signal: "14 clients - yield / distribution complaints", trend: "worsening", sev: "red", route: "advisory", rmTeam: "Advisory desk · West" },
...
const EXIT_PHRASES: { phrase: string; count: number; wow: number; cohort: string }[] = [
  { phrase: "\u201cdisappointed with returns\u201d", count: 21, wow: 7, cohort: "Alternatives Advisory" },
  { phrase: "\u201cmove my portfolio\u201d", count: 18, wow: 9, cohort: "South - HNI" },
  { phrase: "\u201cfees are too high\u201d", count: 16, wow: 3, cohort: "Emerging HNI" },
  { phrase: "\u201cclose the account\u201d", count: 12, wow: 5, cohort: "South - HNI" },
  { phrase: "\u201cdistributions have slowed\u201d", count: 11, wow: 4, cohort: "Digital-Retail" },
...
function ExitLanguageTracker() {
  ...
  return (
    <SectionCard title="Exit-language phrase tracker" subtitle="Exact attrition phrases detected in client conversations - count & week-on-week move" accent={T.red} aiPill>
      {/* One shared grid so phrase/bar/meta columns align across rows (per-row grids caused stair-step bars). */}
      <div
        style={{
          display: "grid",
          gridTemplateColumns: "minmax(200px,1.55fr) minmax(140px,1.45fr) 28px minmax(150px,auto)",
          columnGap: 10,
          rowGap: 9,
          alignItems: "center",
        }}
      >
        {EXIT_PHRASES.map((p) => (
          <div key={p.phrase} style={{ display: "contents" }}>
            <span style={{ fontSize: 11.5, color: T.text, fontWeight: 600, minWidth: 0 }}>{p.phrase}</span>
            <div style={{ height: 16, borderRadius: 5, background: T.track, overflow: "hidden", minWidth: 0 }}>
              <div style={{ width: `${(p.count / max) * 100}%`, height: "100%", background: T.red, display: "flex", alignItems: "center", paddingLeft: 7, minWidth: p.count > 0 ? 28 : 0 }}>
...
const COMPETITOR_MENTIONS: { name: string; count: number; context: string; tiedToExit: number; t: string }[] = [
  ...
  { name: "Waterfield Advisors", count: 9, context: "lower fees", tiedToExit: 44, t: "amber" },
  { name: "Kotak Private", count: 7, context: "digital experience", tiedToExit: 29, t: "amber" },
  { name: "Nuvama", count: 5, context: "research quality", tiedToExit: 40, t: "amber" },
  { name: "Julius Baer", count: 4, context: "RM attention", tiedToExit: 50, t: "amber" },
];
  ...
  return (
    <SectionCard title="Competitor mention monitor" subtitle="Rivals clients name in conversations - and how often it co-occurs with exit-intent" accent={T.amber} aiPill>
      ...
      <AIInsightStrip tone="amber">
        360 ONE is the sharpest threat - most-named, and 71% of its mentions sit inside an exit-intent conversation, almost always on the yield / product-access argument. This is a competitive-positioning brief for the Relationship Manager team, not a service issue.
      </AIInsightStrip>
    </SectionCard>
  );
}
...
const ATTRITION_DRIVERS: { driver: string; share: number; wow: number; theme: string; c: string }[] = [
  { driver: "Yield / performance concern", share: 34, wow: 6, theme: "yield below expectation / distributions slowed", c: T.red },
...
function AttritionDriverAnalysis() {
  return (
    <SectionCard title="Attrition-driver analysis" subtitle="Why at-risk clients are at risk - reason-coded from conversation themes, not surveys" accent={T.violet} aiPill>
      ...
      <AIInsightStrip tone="violet">
        Yield/returns and responsiveness together drive 62% of attrition risk - but only responsiveness is inside CX&apos;s control, and it is rising fastest at +8. Fixing call-backs is the highest-leverage retention move you own outright.
      </AIInsightStrip>
    </SectionCard>
  );
}
...
function MatchedHeightRow({
  ...
  // CSS stretch only — ResizeObserver + setState caused max-update-depth loops.
  return (
    <div style={{ display: "grid", gridTemplateColumns: columns, gap: 12, alignItems: "stretch" }}>
      <div style={{ minWidth: 0 }}>{left}</div>
      <div style={{ minHeight: 0, minWidth: 0, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        {right}
      </div>
    </div>
  );
...
const D2_AI = NEOGROUP_D2_PROMISE_AI;
const D2_AI_DETAILS = NEOGROUP_D2_PROMISE_DETAILS;
...
const GRIEVANCE_ROWS: { cat: string; recv: number; closed: number; open: number; fcr: number; reopen: number; tat: number }[] = [
  ...
  { cat: "Settlement / distribution", recv: 310, closed: 271, open: 39, fcr: 70, reopen: 8, tat: 2.6 },
...
const BROKEN_LOOP: { type: string; chases: number; contacts: number; ch: string }[] = [
  ...
  { type: "KYC completion", chases: 2.2, contacts: 95, ch: "chatmessages" },
  { type: "Statement dispatch", chases: 1.9, contacts: 120, ch: "social media" },
];
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 12 }}>
      <CCWell accent={T.green} title="Automation rate" sub="share of eligible requests auto-handled">
        ...
        <div style={{ height: 92, minWidth: 0 }}>
          <ResponsiveContainer width="100%" height="100%" debounce={50}>
...
const D3_RECS: { rec: string; impact: string; owner: string; t: string }[] = [
  { rec: "Move balance & statement admin to self-serve", impact: "Rs 1.2 Cr / yr cost-to-serve saving", owner: "digital", t: "green" },
  ...
  return (
    <SectionCard title="Straight-through resolution" subtitle="Fulfilment records - share resolved end-to-end without manual handling" accent={T.blue} aiPill>
      ...
      <AIInsightStrip tone="blue">
        Grievance resolution is only 19% straight-through - the heaviest admin still sitting on assisted desks. Automating acknowledgement and status clears that load so RMs are not chasing paperwork, and the fulfilment record confirms both the cost and the capacity returned to advice.
      </AIInsightStrip>
    </SectionCard>
  );
}

const CTS_ROWS: { ch: string; short: string; perReq: number; volume: string; trend: number }[] = [
  { ch: "email", short: "email", perReq: 12, volume: "11,530", trend: -8 },
  { ch: "chatmessages", short: "chat", perReq: 28, volume: "14,860", trend: -3 },
  { ch: "trouble tickets", short: "tickets", perReq: 46, volume: "9,240", trend: 2 },
  { ch: "Voice", short: "Voice", perReq: 142, volume: "18,420", trend: 6 },
  { ch: "social media", short: "social", perReq: 88, volume: "7,180", trend: 4 },
];

const CTS_BAR_COLORS = [T.green, T.green, T.amber, T.red, T.amber];

function CostToServePanel() {
  const lowest = CTS_ROWS.reduce((a, b) => (a.perReq <= b.perReq ? a : b));
  const highest = CTS_ROWS.reduce((a, b) => (a.perReq >= b.perReq ? a : b));
  const spread = Math.round(highest.perReq / lowest.perReq);

  return (
    <SectionCard
      title="Cost-to-serve by channel"
      subtitle="Fulfilment cost ledger - rupee per resolved request - trend vs last month"
      accent={T.blue}
      aiPill
      style={{ height: "100%" }}
      bodyStyle={{ display: "flex", flexDirection: "column", height: "100%" }}
    >
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 10, marginBottom: 12, flexShrink: 0 }}>
        {[
          { l: "Lowest cost / req", v: `Rs ${lowest.perReq}`, d: `${lowest.trend > 0 ? "+" : ""}${lowest.trend}%`, c: T.green },
          { l: "Highest cost / req", v: `Rs ${highest.perReq}`, d: `${highest.trend > 0 ? "+" : ""}${highest.trend}%`, c: T.red },
          { l: "Voice vs email", v: `${spread}x`, d: `Rs ${lowest.perReq} → ${highest.perReq}`, c: T.cyan },
        ].map((m) => (
          <div key={m.l} style={{ padding: "12px 14px", borderRadius: 12, background: T.inset, border: `1px solid ${m.c}35`, borderLeft: `3px solid ${m.c}`, minWidth: 0 }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: T.muted, textTransform: "uppercase", letterSpacing: 0.6 }}>{m.l}</div>
            <div style={{ fontSize: 20, fontWeight: 800, color: T.text, fontFamily: MONO, marginTop: 4, whiteSpace: "nowrap" }}>{m.v}</div>
            <Mono c={m.c} s={11}>{m.d}</Mono>
          </div>
        ))}
      </div>
      <div style={{ height: 150, flexShrink: 0, minWidth: 0 }}>
        <ResponsiveContainer width="100%" height="100%" debounce={50}>
          <BarChart
            data={CTS_ROWS.map((r) => ({ m: r.short, v: r.perReq, full: r.ch, volume: r.volume, trend: r.trend }))}
            margin={{ top: 6, right: 8, left: -18, bottom: 0 }}
          >
            <CartesianGrid strokeDasharray="3 3" stroke={T.border} />
            <XAxis dataKey="m" tick={{ fill: T.dim, fontSize: 10 }} tickLine={false} />
            <YAxis tick={{ fill: T.dim, fontSize: 9 }} width={30} tickLine={false} axisLine={false} tickFormatter={(v) => `Rs${v}`} />
            <Tooltip
              contentStyle={TIP}
              formatter={(v, _n, item) => {
                const row = item?.payload as { full?: string; volume?: string; trend?: number } | undefined;
                const trend = row?.trend ?? 0;
                return [
                  `Rs ${v} / req · ${row?.volume ?? ""} · ${trend > 0 ? "+" : ""}${trend}%`,
                  row?.full ?? "Cost",
                ];
              }}
            />
            <Bar dataKey="v" radius={[4, 4, 0, 0]}>
              {CTS_BAR_COLORS.map((c, i) => (
                <Cell key={CTS_ROWS[i].ch} fill={c} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      ...
      <div style={{ marginTop: "auto" }}>
        <AIInsightStrip tone="blue">
          Voice is Rs 142 per request - nearly 12x email at Rs 12. Moving admin queries off Voice is the single largest cost-to-serve lever - and it protects RM phone time for advice, not statements. email volume is already the fastest-growing channel.
        </AIInsightStrip>
      </div>
    </SectionCard>
  );
...
function ContainmentValuePanel() {
  return (
    <SectionCard
      title="Containment value - automation ROI"
      subtitle="Fulfilment + cost ledger - rupee removed from assisted channels, month-to-date"
      accent={T.blue}
      aiPill
      style={{ height: "100%" }}
      bodyStyle={{ display: "flex", flexDirection: "column", height: "100%" }}
    >
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 10, marginBottom: 12, flexShrink: 0 }}>
        {[["Containment MTD", "Rs 1.84 Cr", "+18%", T.green], ["Requests deflected", "48,200", "+12%", T.cyan], ["Cost avoided / req", "Rs 38", "+6%", T.green]].map(([l, v, d, c]) => (
          <div key={l as string} style={{ padding: "12px 14px", borderRadius: 12, background: T.inset, border: `1px solid ${c as string}35`, borderLeft: `3px solid ${c as string}`, minWidth: 0 }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: T.muted, textTransform: "uppercase", letterSpacing: 0.6 }}>{l}</div>
            <div style={{ fontSize: 20, fontWeight: 800, color: T.text, fontFamily: MONO, marginTop: 4, whiteSpace: "nowrap" }}>{v}</div>
      ...
      <div style={{ marginTop: "auto" }}>
        <AIInsightStrip tone="blue">
          Rs 1.84 Cr removed month-to-date, led by balance and statement admin cleared off assisted channels. Report self-serve is the smallest bar and the clearest next investment - free more RM time for advice where the conversation layer already shows the demand.
        </AIInsightStrip>
      </div>
    </SectionCard>
  );
...
function cannedAnswer(q: string): ReactNode {
    ...
    return (
      <AiReply
        ...
        bullets={[
          ...
          { bold: "Top phrases", rest: ": 'move my portfolio', 'close account', 'disappointed with returns' — mostly Voice and social media." },
          { bold: "Sentiment", rest: " in that cell is -0.58; attrition radar scores South × HNI at 78 (hottest intersection)." },
          { bold: "Root cause", rest: ": unmet call-back promises compounding with responsiveness complaints — not yield/product performance." },
  ...
  if (s.includes("mass affluent") || (s.includes("segment") && s.includes("worry")) || s.includes("after south"))
    return (
      <AiReply
        headline="After South Core-HNI, watch Digital-Retail — volume risk before value risk."
        bullets={[
          { bold: "30% negative", rest: " sentiment in Digital-Retail, rising +3 pts — fastest slip of any segment." },
  ...
  if (s.includes("sentiment") || s.includes("slipping"))
    return (
      <AiReply
        headline="Sentiment is slipping fastest in Digital-Retail; Private-UHNI is the control."
        bullets={[
          { bold: "Digital-Retail", rest: ": 30% negative, +3 pts — highest negative share and rising." },
          ...
          { bold: "Next step", rest: ": fix responsiveness in Digital-Retail before patterns replicate in HNI." },
        ]}
      />
    );
    ...
    return (
      <AiReply
        ...
        bullets={[
          ...
          { bold: "Biggest lever", rest: ": Voice at Rs 142 vs email at Rs 12 — deflect balance and statement queries." },
    ...
    return (
      <AiReply
        ...
        bullets={[
          ...
          { bold: "Parallel play", rest: ": Voice-to-chatmessages deflection for cost — important, not the retention emergency." },
        ]}
      />
    );
    ...
    return (
      <AiReply
        ...
        bullets={[
          ...
          { bold: "Exit language", rest: " rare outside the flagged Alternatives Advisory cluster." },
  ...
  if (s.includes("competitor") || s.includes("switch") || s.includes("advisor"))
    return (
      <AiReply
        ...
        bullets={[
          { bold: "South + Alternatives Advisory", rest: ": highest concentration of 'switching advisor' mentions." },
  ...
  if (s.includes("ewm") || s.includes("advisory") || s.includes("returns") || s.includes("yield") || s.includes("distribution"))
    return (
      <AiReply
        headline="Alternatives Advisory is the second hotspot — yield / distribution language, not service friction."
        bullets={[
          { bold: "14 clients", rest: " using yield-disappointment / distribution-slowdown language this week." },
          { bold: "Top phrase", rest: ": 'disappointed with returns' (+7 WoW) — concentrated in Alternatives Advisory conversations." },
          { bold: "Different from South-HNI", rest: ": yield/credit product concern more than call-back breach — route to advisory desk." },
          { bold: "Attrition radar", rest: ": Alternatives Advisory cluster scores elevated but below South-HNI (78)." },
          { bold: "Next step", rest: ": advisory evidence pack + yield/distribution review outreach — human approves." },
        ]}
      />
    );
  ...
  return (
    <AiReply
      ...
      bullets={[
        { bold: "Channels", rest: ": email, chatmessages, trouble tickets, Voice, social media." },
...
function FloatingAI({ firmName }: { firmName: string }) {
        ...
        <div style={{ position: "fixed", right: 22, bottom: 84, zIndex: 60, width: 400, maxWidth: "calc(100vw - 44px)", height: 540, maxHeight: "calc(100vh - 120px)", background: T.card, border: `1px solid ${T.btn}`, borderRadius: 16, display: "flex", flexDirection: "column", overflow: "hidden", boxShadow: "0 20px 60px rgba(0,0,0,.6)" }}>
          <div style={{ padding: "12px 14px", borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 10.5, color: T.muted }}>Conversation-grounded · {firmName} CX</div>
            </div>
            <button
              type="button"
              onClick={close}
...
/* ============ ROOT ============ */
export function NeoGroupClientExperienceDashboard({
  onExit,
  firmName = "Neogroup",
}: {
  onExit?: () => void;
  /** Brand line shown in chrome (Neo Wealth & Asset Management). */
  firmName?: string;
}) {
  ...
  return (
    <div className="neo-cx" style={{ background: T.bg, color: T.text, minHeight: "100vh", fontFamily: "Inter, ui-sans-serif, system-ui, -apple-system, 'Segoe UI', sans-serif", fontWeight: 600, display: "grid", gridTemplateColumns: "72px 1fr" }}>
      <style>{`
        .neo-cx *{box-sizing:border-box}
        .neo-cx .fade{animation:neocxf .22s ease-out}@keyframes neocxf{from{opacity:.3;transform:translateY(6px)}to{opacity:1;transform:none}}
        .neo-cx .bigcard{transition:transform .16s ease,box-shadow .16s ease}.neo-cx .bigcard:hover{transform:translateY(-3px);box-shadow:0 16px 40px rgba(0,0,0,.5)}
        .neo-cx button:focus-visible,.neo-cx .bigcard:focus-visible,.neo-cx input:focus-visible{outline:2px solid ${T.gold};outline-offset:2px}
        .neo-cx input::placeholder{color:${T.dim}}
        @media (prefers-reduced-motion: reduce){.neo-cx .fade,.neo-cx .bigcard{animation:none;transition:none}}
        .neo-cx .overview-cards{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}
        @media (max-width:1100px){.neo-cx .overview-cards{grid-template-columns:1fr}}
      `}</style>
      ...
      <main style={{ padding: "16px 22px 36px", overflow: "auto", minWidth: 0 }}>
        {screen === "overview" && <NeoOverview go={go} firmName={firmName} />}
        {screen === "d1" && <Drill1 go={go} />}
        {screen === "d2" && <Drill2 go={go} />}
        {screen === "d3" && <Drill3 go={go} />}
      </main>
      <FloatingAI firmName={firmName} />
    </div>
  );
}

export default NeoGroupClientExperienceDashboard;

NeoGroupClientExperienceDashboard.displayName = "NeoGroupClientExperienceDashboard";