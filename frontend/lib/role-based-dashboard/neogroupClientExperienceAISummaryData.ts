/**
 * Neo Group · Head of Client Experience — AI Retention / Promise walls.
 * Neogroup-only copy. Do not import from nuvama* data files for this route.
 */

import type { FCIInsight, FCIInsightDetails } from "@/components/FCI/AISummaryWall";

export const NEOGROUP_D1_RETENTION_AI: FCIInsight[] = [
  {
    id: "d1a1",
    severity: "critical",
  ...
  },
  {
    ...
    title: "Digital-Retail sentiment slipping fastest",
    message:
      "Negative-sentiment share is highest and rising in Digital-Retail, the largest interaction pool - a volume risk even if per-client value is lower.",
    trend: "up",
    change: 3,
    metrics: {
      volume: 30,
...
export const NEOGROUP_D1_RETENTION_DETAILS: Record<string, FCIInsightDetails> = {
  d1a1: {
    rootCause:
  ...
  d1a3: {
    rootCause:
      "Service model is thinner for Digital-Retail; routine friction goes unresolved and accumulates into detractor sentiment.",
...
export const NEOGROUP_D2_PROMISE_AI: FCIInsight[] = [
  {
    id: "d2a1",
    severity: "critical",
    category: "sla-breach",
...
export const NEOGROUP_D2_PROMISE_DETAILS: Record<string, FCIInsightDetails> = {
  d2a1: {
    rootCause:
